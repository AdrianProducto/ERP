Feature: Generar nueva dispersión
  ...
  Necesito poder generar registros de disperciones para anticipos
  ...
  Scenario: Solicitar información para generar dispersión
    ...
    Then el sistema debe solicitar la siguiente información:
      ...
      | Descripción         | Texto    | Sí          | Letras, números y caracteres especiales                        | 30  |              |                                                          |
      | Fecha               | Fecha    | Sí          | Fecha menor o igual a la actual                                |     | fecha_actual |                                                          |
      | Cuenta Bancaria     | Catalogo | Sí          | Cuentas asociadas a los bancos con Layout de anticipos cargado |     |              | Catalogo de cuentas bancarias                            |
    ...
    And presenta una lista con solo anticipos autorizados
    And que no estén relacionados a otra dispersión
    And que no tengan referencia bancaria
    And cuya fecha sea igual al dia actual
    And por cada anticipo presenta la siguiente información:
      | Folio                 |
      | Fecha                 |
      | Operador              |
      | Importe               |
  ...
  Scenario: Búsqueda de anticipos a incluir
    Given el usuario captura un rango de fechas de búsqueda
    When el usuario presiona el boton "Actualizar"
    Then sistema encuentra anticipos autorizados
    And que no estén relacionados a otra dispersión
    And que no tengan referencia bancaria
    And cuya fecha esté dentro del rango seleccionado
    And actualiza el listado anticipos para presentar las coincidencias
  ...
  Scenario: No se encontraron coincidencias
    ...
    """ 
    And el listado de anticipos se muestra en blanco
  ...
  Scenario: Generación exitosa de la dispersión
    ...
    And existe al menos un anticipo seleccionado
    ...
    And relaciona los anticipos seleccionas con la dispersion registrada
