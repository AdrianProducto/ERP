Feature: Generar de archivo TXT para dispersión bancaria Banorte
  ...
  Background:
    ...
    And existe una liquidación con folio e importe total
    And existe una fecha de aplicación capturada
    And el RFC de la empresa está configurado en Parámetros Generales

  Scenario: Generar archivo TXT para dispersión de liquidaciones
    ...
    And debe tener la siguiente estructura por cada liquidación incluida:
      | Operación              |
      | Clave                  |
      | Cuenta origen          |
  ...
  #OBTENER IMPORTE
  Scenario: Obtener importe de la liquidación
    Given el sistema este generando el archivo
    When este obteniendo la información para el dato importe
    Then el sistema debe tomar el importe del campo neto a pagar de la liquidación
  ...
  Scenario: Obtener referencia de la operación
    ...
    Then el sistema debe tomar el "Folio" correspondiente a la liquidación
